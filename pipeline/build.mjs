...
// Tirana: ONE feed published by the Municipality of Tirana (pt.tirana.al),
// 27 bus lines — the whole public network of the city, which has no tram, no
// trolleybus and no metro. Line numbers carry a letter for the branch (1A,
// 3B, 12A/12B) exactly as the buses do.
...
// Exact-match repairs for poles a feed names badly. Tirana needs none: of
// 490 stop names only fourteen shout, and each is an acronym the city itself
// shouts (TEG the shopping centre, QTU the other one, PTUU the university
// polyclinic).
const NAME_FIX = {};
...
// ONE feed, ONE cfg.
//
// The Municipality of Tirana publishes the city network as a single GTFS with
// shapes (CC-BY-SA). Everything in it is route_type 3 and one agency, so the
// map is one mode in one colour — the family's plainest shape, shared with
// Plovdiv.
const LBL = new Map();
